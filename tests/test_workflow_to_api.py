...
from ComfyTV.runners.vendor.workflow_to_api import (
...
class TestMalformedInputHardening:
    ...
    def test_single_bad_node_does_not_abort_conversion(self, object_info, caplog):
        ...
        from ComfyTV.runners.vendor import workflow_to_api as mod
        ...
        try:
            with caplog.at_level(logging.ERROR, logger="ComfyTV.runners.vendor.workflow_to_api"):
                result = convert_ui_to_api(workflow, object_info)
        finally:
...
class TestFrontendParity:
    ...
    def test_group_node_workflow_emits_warning(self, object_info, caplog):
        ...
        with caplog.at_level(logging.WARNING, logger="ComfyTV.runners.vendor.workflow_to_api"):
            convert_ui_to_api(workflow, object_info)
...
class TestDynamicComboAfterControlMarker:
    ...
    def test_deeply_nested_dynamic_combos_do_not_recurse_forever(self, caplog):
        ...
        from ComfyTV.runners.vendor.workflow_to_api import _MAX_DYNAMIC_COMBO_DEPTH
...
class TestDynamicPrompts:
    ...
    def test_picks_one_option_per_group(self):
        with patch("ComfyTV.runners.vendor.workflow_to_api.random.choice", side_effect=lambda opts: opts[0]):
            assert process_dynamic_prompt("{option1|option2}") == "option1"
        with patch("ComfyTV.runners.vendor.workflow_to_api.random.choice", side_effect=lambda opts: opts[-1]):
