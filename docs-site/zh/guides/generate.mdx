---
title: "生成内容"
translationSourceHash: "05b916d8492448a9"
---
...
- **workflow** —— 自带两个家族：
  ...
  - **MiniMax H3**（一趟直出**带声音**的视频——环境声、拟音甚至对白）：
    ...
    - `Local MiniMax H3 R2V` —— 多参考：接入多张图（角色设定、场景、道具、风格）和可选的参考音频,在提示词里用 `@image_N` / `<Audio N>` 指名引用,跨镜锁定身份与连续性。H3 工作流内置 **Lightning** LoRA,生成耗时约减半（见 [models.zh.md](/guides/models)）。
...
输出是一个音频文件。之后[音频套件](/guides/video-and-audio)（均衡、分轨、混响……）和 [Music 节点](/guides/making-music)都能接手。
...
三者所需模型文件见 [models.zh.md](/guides/models)；也可以 Link 或 Upload 你自己的 `model` kind 工作流。节点内嵌轨道预览；旋转视角后会自动截图进它的图片输出，下游图像 stage 可以直接消费模型的外观。GLB 本体流向 3D 节点——Scene3D、网格操作、材质，详见节点参考。
