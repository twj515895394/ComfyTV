...
Output is a single audio file. From there the [audio suite](/guides/video-and-audio) (EQ, stem split, reverb, …) and the [Music nodes](/guides/making-music) can pick it up.
...
Model files for all three are listed in [models.md](/guides/models); you can also link or upload your own `model`-kind workflow. The node embeds an orbit preview; after you orbit, it auto-captures a screenshot into its image output, so downstream image stages can consume the model's look. The GLB itself flows to the 3D nodes — Scene3D, mesh ops, materials; see the Node Reference.
